...
public class MainActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        //안드로이드에서도 ListView와 데이터를 연결해주는 객체가 지원되는데, 이 객체를 가리켜 중간자라 해서 어댑터라 한다
        //ArrayAdapter adapter = new ArrayAdapter(this, android.R.layout.simple_list_item_1, fruit); //현재 개발중인 클래스, 등록된 resources, 실제 보여질 데이터

        //우리가 원하는 레이아웃이 아닌 경우 직접 만들면 된다!
        //ArrayAdapter 말 그대로 하나의 배열을 이용하여 단일 아이템만 표현할 수 있는 어댑터이다
        //복합적인 뷰를 표현할때는 어떻게 해야하는가? Swing 개발할 때처럼 Model클래스를 재정의하듯이 Adapter클래스를 재정의해야함 ->내일
