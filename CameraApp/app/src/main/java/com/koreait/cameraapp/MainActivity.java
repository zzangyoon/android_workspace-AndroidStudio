...
public class MainActivity extends AppCompatActivity {
    ...
    public void openCamera(View view){
        //다른 앱을 호출하려면, 암시적 인텐트 이용해야 한다! 왜?
        //그 앱을 제작한 개발자가 만든 액티비티명을 모르기 때문에
        //또한, 다른 앱을 접근할 수 없기 때문에 안드로이드 시스템에게 간접적으로 열어줄것을 부탁해야하기 때문

        /*
            2015년 마시멜로 버전 이후부터는 보안이 강화되었기 때문에
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        if(requestCode == CAMERA_REQUEST){  //카메라 요청에 대한 응답인지 여부
            ...
            if(grantResults[0] == PackageManager.PERMISSION_GRANTED){   //허락
                Intent intent = new Intent(MediaStore.ACTION_IMAGE_CAPTURE);    //카메라앱 호출해달라는 의도
                startActivityForResult(intent, CAMERA_REQUEST); //앱 띄우기
            }else{
    ...
    //요청 후 결과를 가져올게 있을때 이 메서드 재정의... (카메라앱 or 갤러리앱... 모른다)
