...
public class FormActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    public void sendAndGet(){
        ...
        //매개변수
        startActivityForResult(intent, REQUEST_CODE);   //(전달할 데이터, 요청 구분코드)
    }

    //다시 되돌아 전달한 데이터 받기! 이미 정해진 메서드 오버라이드 하자
    @Override
    ...
    public void callPhone(){
        //우리가 제작중인 현재 앱 안에 있는 액티비티가 아니라, 외부 앱의 액티비티명이 뭘까?
        //결론 : 외부앱의 액티비티명은 우리가 알수고 없고, 알필요도 없다...
        // 이때는 명시적이 아닌 암시적, 묵시적(implicit) 인텐트를 사용해야 한다... 한마디로 얼버무려야한다
        //안드로이드의 Intent가 자체적으로 가진 상수들이 있다. 이 상수들을 이용하면 안드로이드가 해석하는데 도움이 된다
        Intent intent = new Intent(Intent.ACTION_DIAL, Uri.parse("tel:01077779999"));
