...
public class ResultActivity extends AppCompatActivity {
    ...
    //나를 호출한 액티비티에 데이터를 전달하자! 주로 결과전달시 사용됨
    ...
    public  void send(){
        //보낼 데이터 구성하기
        Intent intent = new Intent();   //명시하지 않은이유 ?
        // 우리가 원하는건 새롭게 생성된 액티비티가 아니기 때문... 즉 기존 액티비티와 소통해야함
        ...
        //RESULT_OK 상수는 이미 액티비티가 보유한 상수로서, 성공의 의미를 담고있다
        //마치 http통신시 서버코드(200) 처럼
        setResult(this.RESULT_OK, intent);      //나를 호출한 액티비티에 결과 전달하기!
        //나를 호출한 액티비티 누군지 시스템이 알고있다.. 따라서 이 메서드만 호출하면됨
        finish();
    }
}
