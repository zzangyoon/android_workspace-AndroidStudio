...
public class ConnectManager extends Thread{
    ...
    String requestUrl;
    String data;
    //이 객체를 생성하는 자는 주소와 제이슨데이터를 넘겨야 한다
    ...
    public int requestByGet(){  //Get방식으로 요청을 시도하는 메서드
        ...
        try{
            ...
            code = con.getResponseCode();	//서버로부터 받은 응답코드 반환 (이 시점엔 이미 서버에 요청을 완료 후 응답도 받은 상태)
            Log.d(TAG, "서버로 부터 받은 응답코드는 "+code);

            //이 시점이 바로 통신이 완료된 시점이므로, 여기서 무언가를 해야한다
            //메인액티비티가 보유한 printData
