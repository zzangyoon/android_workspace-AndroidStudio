...
//자주 사용되는 비동기 객체가 지원된다... but 2019 말에 Deprecated...
...
public class MyAsync extends AsyncTask<String, Void, String> {
    ...
    //비동기 요청 전에 하고싶은 작업 (메인쓰레드로 수행 : UI 제어가능)
    ...
    //요청 후 받은 응답데이터 처리 (메인쓰레드로 수행 : UI 제어가능)
    //doInBackground() 메서드에서의 반환값이 아래의 onPostExecute() 메서드의 인수로 전달(시스템에 의해)
    protected void onPostExecute(String data) {
        Log.d(TAG, "background 요청 후 전달받은 데이터는 "+data);
        ...
        try {
            ...
            for(int i=0; i<jsonArray.length(); i++){
                ...
                ImageDownLoaderAsync downLoaderAsync = new ImageDownLoaderAsync(galleryFragment, gallery);
                downLoaderAsync.execute("http://172.30.1.4:7777/images/"+gallery.getFilename());
            ...
            galleryFragment.gridView.invalidate();  //UI갱신
